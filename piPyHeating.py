...
def processTemp():
    ...
    if state == 0:
        try:
            ...
            pass # Will fail if pump is already off
        try:
            boiler.close()
        except:
            pass # Will fail if boiler is already off
        led.off()
    if state == 1:
        pumpOn = 0
        boilerOn = 0
...
def onAlarm(signum, frame):
    for sensor in sensors:
        try:
            #TODO: w1termsensor should raise exception but (https://github.com/timofurrer/w1thermsensor/issues/75)
            result = ds_sensors[sensor].get_temperature()
            if result == 85.0:
                result = ds_sensors[sensor].get_temperature()
            if result != 85.0:
                sensors[sensor]['value'] = result
        except:
